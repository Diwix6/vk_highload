...
| Работа со связями | Связанные данные часто хранятся внутри одного документа, что уменьшает потребность в JOIN mongodb | JOIN — нативный механизм, поддерживаемый мощным планировщиком postgresql+1       |
...
| Сильная сторона | Быстрые операции над отдельными документами и высоконагруженный read-heavy сценарий dev+1    | Сложные запросы, агрегации, аналитика и транзакционная строгость postgresql+1                     |
