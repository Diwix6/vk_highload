...
|---|---|---|
| **MySQL** | Очень популярна | Слабая работа с JSON, нет нормальных GIN-индексов, плохо подходит для CRDT |
| **MongoDB** | Нативный JSON, горизонтальное шардирование | Нет полноценных ACID-транзакций, слабые JOINы, меньше подходит для сложных операций |
| **CockroachDB** | Distributed SQL, автоматическое шардирование | Слишком высокая задержка при записях |
...
2. **ACID-транзакции** — изменение роли участника, удаление доски, создание объекта с правами — всё требует атомарности. 
3. **Горизонтальное масштабирование через Citus** — расширение PostgreSQL, которое добавляет шардирование по `board_id` без изменения SQL-интерфейса. 
4. **Workset в RAM** — 2.7 млрд строк `BOARD_OBJECT` разбиты на 340 шардов по ~8 млн строк; рабочий набор каждого шарда
