...
| `BOARD` | `title` | Text Index | ~20 токенов × 8 Б | 27 млн | **~4 ГБ** | Full-text поиск |
...
| `COMMENT` | `body` | Text Index | ~10 токенов × 8 Б | 270 млн | **~21 ГБ** | Full-text поиск |
...
| `BOARD` | MongoDB | **Не шардируется** — 27 млн строк × 10 КБ ≈ 270 ГБ, тривиально для одного кластера | 1 Master + 2 Slave | Метаданные досок нужны при поиске, инвайтах — cross-shard join по board_id усложнит |
...
| PostgreSQL (boards sharded) | 1 primary + 2 replica на каждый шард (340 шардов × 3 ноды) |
...
**Отказ Export Service:** Работа на доске не прерывается. Задачи экспорта накапливаются в Kafka и обрабатываются после восстановления сервиса.
 
**Отказ Redis (sessions):** Пользователи переавторизуются. JWT токены проверяются напрямую без Redis как fallback.
 
### 10.5 Graceful Shutdown
 
При обновлении кода сервис завершается плавно:
 
1. Kubernetes отправляет сигнал SIGTERM
...
Это гарантирует, что видеозвонки и работа на доске не прерываются при деплое.
